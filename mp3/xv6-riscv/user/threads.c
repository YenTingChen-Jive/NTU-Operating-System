...
void schedule(void){
    ...
    {
        // printf("execute the first thread in wait_queue at time==0\n");
        return ;
    }
    else
    {
        current_thread = current_thread->next;
        // printf("change current to %d\n", current_thread->ID);
    }
    ...
    // ... implement SJF here.
        if (current_thread->is_exited || current_thread->is_yield || is_thread_start == 0) {
            struct thread *shortest = current_thread->next;
            struct thread *current = current_thread->next;
            for (int i = 1; i < id; i++) {
                if (current->remain_execution_time != 0) {
                    if (current->remain_execution_time == shortest->remain_execution_time && current->ID < shortest->ID) {
                        shortest = current;
                    }
                    else if (current->remain_execution_time < shortest->remain_execution_time) {
                        shortest = current;
                    }
                }
